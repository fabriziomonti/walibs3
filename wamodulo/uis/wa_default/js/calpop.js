...
Calendar.prototype.show = function() {
	...
	ggWinContent += ("<A HREF=\"javascript:void(0);\" " +
		...
		");" +
		"\" class=\"lcalpop\">&lt;&lt;<\/A></TD><TD ALIGN=center>");
	ggWinContent += ("<A HREF=\"javascript:void(0);\" " +
	...
	ggWinContent += ("<A HREF=\"javascript:void(0);\" " +
		...
		"'" + this.gReturnItem + "', '" + this.gMonth + "', '" + (parseInt(this.gYear)+1) + "', '" + this.gFormat + "'" +
		");" +
		"\" class=\"lcalpop\">&gt;&gt;<\/A></TD></TR></TABLE><BR>");
...
Calendar.prototype.showY = function() {
	...
	for (i=0; i<12; i++) {
		...
		ggWinContent += "<TD ALIGN='center' VALIGN='top' class='calmhdr'>";
		this.gMonth = i;
...
Calendar.prototype.cal_header = function() {
	...
	vCode = vCode + "<TR>";
	vCode = vCode + "<TD WIDTH='14%' class=\"calhdr\">Dom</TD>";
	vCode = vCode + "<TD WIDTH='14%' class=\"calhdr\">Lun</TD>";
	vCode = vCode + "<TD WIDTH='14%' class=\"calhdr\">Mar</TD>";
	vCode = vCode + "<TD WIDTH='14%' class=\"calhdr\">Mer</TD>";
	vCode = vCode + "<TD WIDTH='14%' class=\"calhdr\">Gio</TD>";
	vCode = vCode + "<TD WIDTH='14%' class=\"calhdr\">Ven</TD>";
	vCode = vCode + "<TD WIDTH='14%' class=\"calhdr\">Sab</TD>";
	vCode = vCode + "</TR>";
	...
	for (j=vFirstDay; j<7; j++) {
		vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j) + "><FONT FACE='" + fontface + "'>" + 
			"<A HREF='javascript:void(0);' " + 
				...
				// inizio modificato bicio
				// "';ggPosX=-1;ggPosY=-1;nd();nd();\">" +
				"';updateDateCB('" + this.gReturnItem + "');ggPosX=-1;ggPosY=-1;nd();nd();\" class=\"lcalpop\">" +
				// fine modificato bicio
		...
		for (j=0; j<7; j++) {
			vCode = vCode + "<TD WIDTH='14%'" + this.write_weekend_string(j) + "><FONT FACE='" + fontface + "'>" +
				"<A HREF='javascript:void(0);' " +
					...
					"';updateDateCB('" + this.gReturnItem + "');window.scroll(0,ggPosY);ggPosX=-1;ggPosY=-1;nd();nd();\" class=\"lcalpop\">" +
					// fine modificato bicio
...
function Build(p_item, p_month, p_year, p_format) {
	...
	if (ggPosX == -1 && ggPosY == -1) {
		overlib(ggWinContent, AUTOSTATUSCAP, STICKY, CLOSECLICK, CSSSTYLE,
			...
			CAPTION, "Seleziona un giorno", OFFSETX, 20, OFFSETY, -20);
		...
		overlib(ggWinContent, AUTOSTATUSCAP, STICKY, CLOSECLICK, CSSSTYLE,
			TEXTSIZEUNIT, "pt", TEXTSIZE, 8, CAPTIONSIZEUNIT, "pt", CAPTIONSIZE, 8, CLOSESIZEUNIT, "pt", CLOSESIZE, 8,
			CAPTION, "Seleziona un giorno", FIXX, ggPosX, FIXY, ggPosY);
		}
...
// aggiunto da bicio
function updateDateCB(passCtrlName)
{
	// cerco il basename del controllo
	var elems = passCtrlName.split('.');
	var forma = elems[0];
	var pcName = elems[2];
	var form = document.getElementById(forma);
	elems = pcName.split('pass_');
	basename = elems[1];
	elems = form.elements[pcName].value.split('-');
	anno = elems[0];
	mese = elems[1];
	giorno = elems[2];
	var gcb = form.elements['wamodulo_giorno_' + basename];
	var mcb = form.elements['wamodulo_mese_' + basename];
	var acb = form.elements['wamodulo_anno_' + basename];
	gcb.selectedIndex = giorno;
	mcb.selectedIndex = mese;
	for (var i=0; i < acb.options.length; i++)
		{
		if (acb.options[i].value == anno)
			{
			acb.selectedIndex = i;
			break;
			}
	...
	myShowCBs();
	if (gcb.onchange)
		gcb.form.wamodulo.controlli[basename].simulaEvento("change");

}
function myShowYearCal(passCtrlName)
{
	myHideCBs();
	ggPosX=1;
	ggPosY=1;
	show_yearly_calendar(passCtrlName);
}
function myShowMonthCal(passCtrlName)
{
	//myHideCBs();
	ggPosX=-1;
	ggPosY=-1;
	show_calendar(passCtrlName);
}
function myShowCBs()
{
