...
<xsl:template match="waapplicazione">
	<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<body >
			...
			<!--mostriamo il menu solo se non siamo in una pagina figlia-->
			<xsl:if test="pagina/elementi/elemento[nome = 'wadoc_menu'] and not(pagina/elementi/elemento[nome = 'nome_pagina_ritorno'])">
				<xsl:text>&#10;</xsl:text>
				<div class="waapplicazione_wadoc_menu">
					<xsl:value-of disable-output-escaping="yes" select="pagina/elementi/elemento[nome = 'wadoc_menu']/valore" />
				</div>
				<xsl:text>&#10;</xsl:text>
			</xsl:if>
			...
			<xsl:if test="pagina/elementi/elemento[nome = 'titolo']">
				...
				<div class="waapplicazione_titolo">
					...
					<xsl:if test="pagina/elementi/elemento[nome = 'nome_pagina_ritorno']">
						<br />
						<xsl:value-of select="pagina/elementi/elemento[nome = 'nome_pagina_ritorno']/valore" />
					</xsl:if>
				</div>
			...
			<!-- creazione degli altri elementi costitutivi della pagina (tabelle, moduli, testo libero, ecc.) -->
			<xsl:for-each select="pagina/elementi/elemento[nome != 'titolo' and nome != 'wadoc_menu' and nome != 'nome_pagina_ritorno']">
				<xsl:text>&#10;</xsl:text>
